...
Changelog:
    2016-04-06 - Modified by Seraphim Sense Ltd. to show only Angel Sensor
                 devices, filtering out any other advertising packets
...
"""
    ...
    p.set_defaults(port="COM10", baud=115200, interval=0xC8, window=0xC8, display="trpsabd", uuid=[], mac=[], rssi=0, active=True, quiet=False, friendly=True)
...
def get_local_name(ad_fields):
    # Example of ad_fields argument: [[1, 6], [9, 65, 110, 103, 101, 108, 32, 83, 101, 110, 115, 111, 114, 32, 48, 53, 52, 53, 57]]
    local_name = ''
    for f in ad_fields:
        if f[0] == 9:
            local_name = ''.join(chr(i) for i in f[1:])
            break
    return local_name
...
known_macs = []
def bgapi_parse(b):
    global bgapi_rx_buffer, bgapi_rx_expected_length, known_macs
            ...
            if packet_class == 0x06: # gap
                if packet_command == 0x00: # scan_response
                    ...
                    if display:
                        ...
                        # The original script was modifed here to filter out
                        # all but Angel Sensor devices.
